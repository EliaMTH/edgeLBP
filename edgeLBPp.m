%% Edge Local Binary Pattern (edgeLBP), parallel
%
...
%
% This function requires the Parallel Computing Toolbox. If you do not have
% it, please use the edgeLBP.m function instead. 
%
...
%%
function [a1,data]=edgeLBPp(v,f,R,P,nrad,h)
...
% the edgeLBP values are now computed. This intricate structure is required
% in order to use the Parallel Computing Toolbox. 
...
parfor j=1:nv
    if vld(j)==1 
        [tr(j,:),a1(j,:),vld(j)]=parallel_loop(j,prel);
    else
...
%% ------------------------------------------------------------------------
function [ttr,ta1,vld]=parallel_loop(j,prel)
...
%% ------------------------------------------------------------------------
% Getting the sorted set of p_i. This is done two ways: in the standard
